...
if (argv.debug) {
    console.log('Coin-Allocator v' + require('./package.json').version);
...
var steps = [
    ca.getStatus.bind(ca)
];

if (argv.debug) {
    steps.push(function(status, next) {
...
if (argv['compute-gains']) {
    steps.push(
        function(status, next) {
            ...
            ca.getTradeGains(status, function(err, gains) {
                next(err, status, gains);
            });
        }
    );
}
...
steps.push(
    function(status, gains, next) {
        if (typeof gains == 'function') {
            // --no-compute-gains was passed in
            next = gains;
            gains = null;
        }
